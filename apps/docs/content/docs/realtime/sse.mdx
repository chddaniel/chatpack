...
back to [interval refetch](/docs/client/realtime#polling-fallback). It never puts
...
<Callout type="warn">
  ...
  poll instead - `@chatpack/client` falls back on its own, so a serverless deploy needs no
  configuration. Details in [Deployment](/docs/integrations/deployment).
</Callout>
