...
`threadRootMessageId` to put a reply in a thread. Thread replies have their
own page and count. A sender can also set `alsoSendToMain: true` to show the
same reply in the main conversation. See [Message threads](/docs/concepts/message-threads).
