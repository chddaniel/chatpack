...
For a new app that needs live chat, Railway is the easiest default for the
Chatpack API because it runs as a long-lived Node service. Render, Fly.io, and
a conventional server or VM are also options. The frontend can stay on Vercel
while the API runs on one of these hosts. Before calling realtime ready, test
message delivery between two authenticated users in separate sessions.
...
inbox, and alert delivery. See [Message threads](/docs/concepts/message-threads)
