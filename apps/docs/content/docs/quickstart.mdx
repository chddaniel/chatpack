...
The client uses the server's existing cookie session. It does not implement
login, logout, user lookup, or token handling. See the [client guide](/docs/client)
for REST, realtime, React, and plugin usage.
...
A group uses a different route - and always creates, since two groups with the
same members are still two groups:
...
The caller becomes an `admin`, everyone in `userIds` a `member`. Renaming and
membership changes are four admin-only routes - see
[Groups](/docs/concepts/conversations-messages#groups-created-never-found).
...
<Callout type="warn">
  **Chatpack never owns a users table.** Configure `userExists(userId)` to validate direct-chat
  targets and new group participants against your identity store. Omitting the optional hook keeps
  opaque-id behavior.
</Callout>
...
no lost messages. Reactions are the exception: they have no `seq`, so they're
live-only and show up on the next refetch instead. Details in
[Real-time with SSE](/docs/realtime/sse).
