...
join requests, text messages, permissions, read state and unread counts, SSE
with gap-fill, memory and Drizzle storage, Redis event fan-out,
typing/presence/receipts, reactions, quote-replies, participant-scoped search,
the post-persistence mutation hook, the browser client with React hooks and
polling fallback, Filepack-backed file attachments, and project setup with
`@chatpack/cli init`.
...
The invite and join-request routes have no client wrappers yet - call them with
`fetch` against the [REST contract](/docs/reference/rest-api#invite--join-request-routes).
Redeeming an invite publishes the same `participant.added` event the group
...
reference `StorageAdapter` implementation, including both optional capabilities
(search, and the `invites` namespace behind invite links and join requests).
