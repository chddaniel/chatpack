...
Push notification providers, reusable React UI components, true message
threads, and multi-node presence have not shipped.
...
The invite, join-request, and channel routes have no client wrappers yet - call
them with `fetch` against the REST contract
([invites](/docs/reference/rest-api#invite--join-request-routes),
[channels](/docs/reference/rest-api#channel-routes)). Redeeming an invite or
joining a channel publishes the same `participant.added` event the group
mutations do, so the conversation list updates itself.
