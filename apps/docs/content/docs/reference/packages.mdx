...
The current package set covers direct and group conversations, text messages,
permissions, read state and unread counts, SSE with gap-fill, memory and
Drizzle storage, Redis event fan-out, typing/presence/receipts, reactions,
quote-replies, participant-scoped search, the post-persistence mutation hook,
the browser client with React hooks and polling fallback, and project setup
with `@chatpack/cli init`.

File attachments remain pending in
[PR #7](https://github.com/chddaniel/chatpack/pull/7). No attachment package is
part of current `main` or the published package list below. Push notification
providers, reusable React UI components, true message threads, and multi-node
presence have not shipped.
...
| Types                     | `ChatpackOptions`, `ChatpackUser`, `AuthHook`, `PermissionHooks`, `StorageAdapter`, `Transport`, `ChatpackPlugin`, `Conversation`, `ConversationType`, `Participant`, `ParticipantRole`, `Message`, `MessageWithDetails`, `Reaction`, `ReactionSummary`, `MessageRole`, `TelemetryPayload`, ... |
...
| `conversations`, `conversationParticipants`, `messages`, `messageReactions`, `messageSearchTokens` | The individual tables                                      |
