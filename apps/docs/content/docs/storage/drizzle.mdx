...
<Callout type="warn">
  ...
  **Re-run the migration before deploying the upgrade.** Every statement is `IF NOT EXISTS` / `ADD
  COLUMN IF NOT EXISTS`, so re-running the whole script is safe and preserves your data and `seq`
  counters.
</Callout>
