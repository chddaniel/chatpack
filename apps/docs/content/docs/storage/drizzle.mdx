...
<Callout type="info">
  ...
  `chatpack_moderation_reports` and `chatpack_user_bans` are **pure table additions** with no changes
  to existing tables, so they too are safe to apply before deploying the new code.
</Callout>
