...
Supabase projects that expose PostgREST through the Supabase JS client should
use the first-party [Supabase adapter](/docs/storage/supabase). Convex,
Firestore, and other stores without a first-party adapter are supported through
a [custom adapter](/docs/storage/custom-adapter). If you can reach Postgres
with a **connection string** (Neon, RDS, Railway, Fly, Replit, Vercel Postgres),
