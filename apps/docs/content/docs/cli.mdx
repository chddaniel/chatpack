...
message sent on one will not reach listeners on another. Presence is the
exception - its snapshot only knows about the streams open on the node that
answers it, so it stays single-node either way.
