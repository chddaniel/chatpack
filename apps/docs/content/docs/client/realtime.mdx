...
The server's membership events (`participant.added`, `participant.removed`,
`conversation.updated`) are **not** in the client's subscribed set yet, so they
don't reach `subscribe` or the cache. Until they are, refetch after a group
change you initiated, or let the polling loop pick it up.
