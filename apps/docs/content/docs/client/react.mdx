...
The hook requests on every query change. Debounce text input before passing it
to the hook (about 250 ms is a useful starting point); the client also bounds
its per-instance search cache to ten normalized queries.
