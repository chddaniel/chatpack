...
and `seq`), the conversation, `otherParticipantId`, and the completed action.
It runs after the storage write and the internal live broadcast. It cannot
change the message or fail the request. If it throws, Chatpack logs the error
...
receives `send` and `edit` only. Use `afterMessageMutation` for new code.
