...
<Callout type="warn">
  Hooks are **in-process functions, not webhooks**. Chatpack catches hook errors, but it awaits the
  hook before finishing the request, so slow provider calls add message latency. Keep the hook fast.
  For reliable delivery, write jobs to your queue or outbox here, then let a worker check
  preferences and call your provider. See the app's
  [hook](https://github.com/chddaniel/chatpack/blob/main/apps/app/src/lib/chatpack.server.ts) and
  [email and browser push
  delivery](https://github.com/chddaniel/chatpack/blob/main/apps/app/src/lib/thread-delivery.ts)
  examples. (Scope rationale: ADR 0014.)
</Callout>
