/**
 * `@chatpack/adapter-turso` - Drizzle ORM (Turso/libSQL) {@link StorageAdapter}
 * for Chatpack. Real persistence for production (M4).
 *
 * Uses Drizzle's async `@libsql/client` driver and libSQL transaction
 ...
 * import { tursoAdapter } from "@chatpack/adapter-turso";
 *
 * const db = drizzle({ client: createClient({ url: process.env.TURSO_DATABASE_URL! }) });
 * const chat = chatpack({ storage: tursoAdapter(db), auth });
 * ```
 *
 ...
 *   `UPDATE ... SET last_seq = last_seq + 1 RETURNING` - Turso/libSQL row
...
function isBusyError(error: unknown): boolean {
  ...
  const candidate = error as { code?: string; rawCode?: number };
  return candidate.code === "SQLITE_BUSY" || candidate.rawCode === 5;
}

/**
 * Interactive libSQL transactions use a database-wide write lock. A second
 * concurrent writer can receive SQLITE_BUSY instead of waiting, especially
 ...
 * their rows automatically through {@link tursoAdapter}.
 */
export async function backfillMessageSearchTokens(db: DrizzleTursoDatabase): Promise<void> {
  const rows = await db
...
/**
 * Create a Drizzle/Turso/libSQL storage adapter.
 *
 ...
 *
 * @param db - Any Drizzle Turso/libSQL database instance.
 */
export function tursoAdapter(db: DrizzleTursoDatabase): StorageAdapter {
   ...
   * more importantly the order is **stable across reads**. Turso/libSQL gives no
  ...
  async function reloadConversation(conversationId: string): Promise<Conversation> {
    ...
    if (!conversation) {
      throw new Error(`tursoAdapter: unknown conversation "${conversationId}".`);
    }
    return conversation;
  }
  ...
  return {
    ...
    async getOrCreateDirectConversation(
      ...
      // arbiter. Keep conversation and participant rows in one transaction so
      // readers never observe a direct conversation without its membership.
      const created = await withTransaction(db, transactionQueue, async (tx) => {
        const inserted = await tx
          ...
          // targetless Turso/libSQL conflict handler still catches that
          // unique-index conflict and avoids dialect-specific conflict syntax.
          .onConflictDoNothing()
      ...
      if (!row) {
        throw new Error(
          `tursoAdapter: conversation for pairKey "${input.pairKey}" vanished after insert.`,
        );
      }
      const participants = await participantsFor([row.id]);
    ...
    async createGroupConversation(input: CreateGroupConversationInput): Promise<Conversation> {
      ...
      await withTransaction(db, transactionQueue, async (tx) => {
    ...
    async addMessage(input: AddMessageInput): Promise<Message> {
      ...
      // read-modify-write. libSQL locks the row for the duration of the
      // UPDATE, so concurrent sends serialize and each gets a unique seq.
      return withTransaction(db, transactionQueue, async (tx) => {
        ...
        if (!bumped) {
          throw new Error(`tursoAdapter: unknown conversation "${input.conversationId}".`);
        }
    ...
    async updateMessage(input: UpdateMessageInput): Promise<Message> {
      ...
      return withTransaction(db, transactionQueue, async (tx) => {
        ...
        if (!row) {
          throw new Error(`tursoAdapter: unknown message "${input.messageId}".`);
        }
    ...
    async updateLastRead(input: UpdateLastReadInput): Promise<void> {
      ...
      if (updated.length === 0) {
        throw new Error(
          `tursoAdapter: user "${input.userId}" is not a participant of "${input.conversationId}".`,
        );
      }
    },
    ...
    invites: {
      ...
      async listJoinRequests(input: ListJoinRequestsInput): Promise<JoinRequest[]> {
        const rows = await db
          ...
          // across reads (Turso/libSQL guarantees none without a full ORDER BY).
