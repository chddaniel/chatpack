  ...
  const moderation: ModerationStorage = {
    ...
    async createBlock(input) {
      ...
      const [row] = await db
        ...
        .limit(1);
      if (!row) throw new Error("tursoAdapter: block insert returned no row.");
      return toBlock(row);
    },
    ...
    async createMute(input) {
      ...
      const [row] = await db
        ...
        .limit(1);
      if (!row) throw new Error("tursoAdapter: mute insert returned no row.");
      return toMute(row);
    },
    ...
    async createReport(input) {
      ...
      const [row] = await db
        ...
        .limit(1);
      if (!row) throw new Error("tursoAdapter: report insert returned no row.");
      return toReport(row);
    },
