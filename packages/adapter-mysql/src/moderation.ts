  ...
  const moderation: ModerationStorage = {
    ...
    async createBlock(input) {
      ...
      await db
        ...
        .onDuplicateKeyUpdate({ set: { createdAt: sql`${userBlocks.createdAt}` } });
      const [row] = await db
        .select()
        .from(userBlocks)
        .where(
      ...
      const rows = await db
        ...
        .where(
          and(
            ...
            cursor
              ? or(
                  gt(userBlocks.createdAt, cursor.date),
                  and(
                    eq(userBlocks.createdAt, cursor.date),
                    gt(userBlocks.blockedUserId, cursor.id),
                  ),
                )
              : undefined,
          ),
        )
        .orderBy(userBlocks.createdAt, userBlocks.blockedUserId)
        .limit(input.limit + 1);
    ...
    async createMute(input) {
      ...
      await db
        ...
        .values({ ...input, createdAt })
        .onDuplicateKeyUpdate({ set: { createdAt: sql`${conversationMutes.createdAt}` } });
      const [row] = await db
        .select()
      ...
      const rows = await db
        ...
        .where(
          and(
            ...
            cursor
              ? or(
                  gt(conversationMutes.createdAt, cursor.date),
                  and(
                    eq(conversationMutes.createdAt, cursor.date),
                    gt(conversationMutes.conversationId, cursor.id),
                  ),
                )
              : undefined,
          ),
        )
        .orderBy(conversationMutes.createdAt, conversationMutes.conversationId)
        .limit(input.limit + 1);
      const page = rows.slice(0, input.limit);
      const last = page[page.length - 1];
      return {
    ...
    async listReports(input): Promise<ModerationPage<ModerationReport>> {
      ...
      const rows = await db
        ...
        .where(
          and(
            input.status === undefined ? undefined : eq(moderationReports.status, input.status),
            input.targetType === undefined
              ? undefined
              : eq(moderationReports.targetType, input.targetType),
            cursor
              ? or(
                  gt(moderationReports.createdAt, cursor.date),
                  and(
                    eq(moderationReports.createdAt, cursor.date),
                    gt(moderationReports.id, cursor.id),
                  ),
                )
              : undefined,
          ),
        )
        .orderBy(moderationReports.createdAt, moderationReports.id)
        .limit(input.limit + 1);
      const page = rows.slice(0, input.limit);
      const last = page[page.length - 1];
      return {
      ...
      const rows = await db
        ...
        .where(
          and(
            ...
            cursor
              ? or(
                  gt(userBans.createdAt, cursor.date),
                  and(eq(userBans.createdAt, cursor.date), gt(userBans.id, cursor.id)),
                )
              : undefined,
          ),
        )
        .orderBy(userBans.createdAt, userBans.id)
        .limit(input.limit + 1);
      const page = rows.slice(0, input.limit);
      const last = page[page.length - 1];
      return {
...
function decodeCursor(cursor: string | undefined): { date: Date; id: string } | null {
  ...
  try {
    ...
    if (Array.isArray(value) && typeof value[0] === "number" && typeof value[1] === "string")
      return { date: new Date(value[0]), id: value[1] };
  } catch {
