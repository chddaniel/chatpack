...
export function mysqlAdapter(db: DrizzleMysqlDatabase): StorageAdapter {
  ...
  async function pageConversationsByActivity(
    ...
    const cursorFilter = decoded
      ? or(
          lt(conversations.lastActivityAt, new Date(decoded.activityMs)),
          and(
            eq(conversations.lastActivityAt, new Date(decoded.activityMs)),
  ...
  return {
    ...
    async getOrCreateDirectConversation(
      ...
      await db.transaction(async (tx) => {
        await tx
          ...
          .onDuplicateKeyUpdate({ set: { pairKey: sql`${conversations.pairKey}` } });
        const [row] = await tx
      ...
      if (!row)
        throw new Error(
          `mysqlAdapter: conversation for pairKey "${input.pairKey}" vanished after insert.`,
        );
      const participants = await participantsFor([row.id]);
      return {
        conversation: toConversation(row, participants.get(row.id) ?? []),
        created: row.id === id,
      };
    },
    ...
    async addParticipants(input: AddParticipantsInput): Promise<Conversation> {
      if (input.userIds.length > 0) {
        ...
        await db
          ...
          .onDuplicateKeyUpdate({ set: { userId: sql`${conversationParticipants.userId}` } });
      }
    ...
    async listConversations(input: ListConversationsInput): Promise<ListConversationsResult> {
      ...
      return pageConversationsByActivity(
        inArray(conversations.id, membership),
        input.limit,
        input.cursor,
      );
    },
    ...
    async addMessage(input: AddMessageInput): Promise<Message> {
      ...
      return db.transaction(async (tx) => {
        ...
        const [conversation] = await tx
          ...
          .limit(1);
        if (!conversation)
          throw new Error(`mysqlAdapter: unknown conversation "${input.conversationId}".`);
    ...
    async searchMessages(input: SearchMessagesInput): Promise<SearchMessagesResult> {
      ...
      if (cursor) {
        ...
        conditions.push(
          or(
            lt(matches.rank, rank),
            and(eq(matches.rank, rank), lt(messages.createdAt, date)),
            and(eq(matches.rank, rank), eq(messages.createdAt, date), lt(messages.id, id)),
          )!,
        );
      }
      const rows = await db
    ...
    async updateLastRead(input: UpdateLastReadInput): Promise<void> {
      ...
      if (!row)
        throw new Error(
          `mysqlAdapter: user "${input.userId}" is not a participant of "${input.conversationId}".`,
        );
    },
    ...
    async countUnread(input: CountUnreadInput): Promise<Record<string, number>> {
      ...
      const rows = await db
        ...
        .leftJoin(readMessage, eq(readMessage.id, conversationParticipants.lastReadMessageId))
        .where(
          and(
    ...
    async addReaction(input: ReactionInput): Promise<Reaction[]> {
      await db
        ...
        .onDuplicateKeyUpdate({ set: { messageId: sql`${messageReactions.messageId}` } });
    ...
    async listReactionsByMessageIds(messageIds: string[]): Promise<Reaction[]> {
      ...
      const rows = await db
        ...
        .orderBy(
          asc(messageReactions.createdAt),
          asc(messageReactions.messageId),
          asc(messageReactions.userId),
          asc(messageReactions.emoji),
        );
      return rows.map(toReaction);
      ...
      await db.transaction(async (tx) => {
        if (input.userIds.length > 0) {
          ...
          await tx
            ...
            .onDuplicateKeyUpdate({ set: { messageId: sql`${messageMentions.messageId}` } });
          await tx
    ...
    async listMentionsByMessageIds(messageIds: string[]): Promise<MessageMention[]> {
      ...
      const rows = await db
        ...
        .orderBy(
          asc(messageMentions.createdAt),
          asc(messageMentions.messageId),
          asc(messageMentions.userId),
        );
    ...
    invites: {
      ...
      async resolveJoinRequest(input: ResolveJoinRequestInput): Promise<JoinRequest> {
        await db
          .update(joinRequests)
          .set({ status: input.status, resolvedAt: input.resolvedAt, resolvedBy: input.resolvedBy })
          .where(
            and(
              eq(joinRequests.conversationId, input.conversationId),
              eq(joinRequests.userId, input.userId),
        ...
        if (!row)
          throw new Error(
            `mysqlAdapter: no join request from user "${input.userId}" in "${input.conversationId}".`,
          );
        return toJoinRequest(row);
      },
    },
