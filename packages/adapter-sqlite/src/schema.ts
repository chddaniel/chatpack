/**
 * The Chatpack SQLite schema (MVP §8), defined with Drizzle ORM.
 *
 ...
 *   optional `reply_to_message_id` quote pointer (ADR 0013), frozen forward
 *   provenance (ADR 0024), and the `metadata` escape hatch.
 ...
 *   pair so a mention set is durable and replaceable (ADR 0023).
 ...
 * {@link migrationStatements} (one statement per call).
 *
 * @module
 */
...
export const conversations = sqliteTable(
  ...
  (table) => [
    ...
    // The explicit name keeps the partial unique index aligned with the
    // exported migration and makes the conflict behavior easy to inspect.
...
/** Canonical tokens used by the SQLite search implementation. */
export const messageSearchTokens = sqliteTable(
...
/** `chatpack_message_mentions` - one row per (message, user) (ADR 0023). */
 ...
 */
export const conversationInvites = sqliteTable(
...
export const conversationMutes = sqliteTable(
 ...
 * statement per call** - e.g. a host wrapper around `better-sqlite3`:
 *
 * ```ts
 * for (const statement of migrationStatements) await db.run(sql.raw(statement));
 * ```
 */
 ...
 * multi-statement scripts. For hosts that execute one statement per call, use
